...
class IRNN(object):
	def __init__(self, seq_length, hidden_size, lr, activation, name):
		...
		# weight = [var for var in tf.trainable_variables() if "rnn_%s/basic_rnn_cell/weights"%name in var.name][0]
		# bias = [var for var in tf.trainable_variables() if "rnn_%s/basic_rnn_cell/biases"%name in var.name][0]

		# identity = tf.eye(hidden_size)
		# random = tf.random_normal([2, hidden_size], stddev=0.01)
		# matrix = tf.concat([identity, random], axis=0)

		# self.w_assign = tf.assign(weight, matrix)
		# self.b_assign = tf.assign(bias, tf.zeros([hidden_size]))
